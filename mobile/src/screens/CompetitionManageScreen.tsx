...
import { ArrowLeft, Trophy, Target, MapPin, Calendar, Users, QrCode, Edit3, Minus } from 'lucide-react-native';
...
interface CompetitionManageScreenProps {
  deviceId: string;
  competitionId: string;
  onBack: () => void;
  onEdit: (competitionId: string) => void;
  onManageReps: (competitionId: string) => void;
  onShowQR: (competitionId: string) => void;
}

export function CompetitionManageScreen({ deviceId, competitionId, onBack, onEdit, onManageReps, onShowQR }: CompetitionManageScreenProps) {
  ...
  const renderHoleItem = ({ item: holeNum }: { item: number }) => {
    ...
    const Icon = cfg.icon;
    return (
      <View style={[styles.holeItem, { backgroundColor: cfg.bg }]}>
  ...
  return (
    <View style={styles.screen}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>コンペ管理</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.actionsContainer}>
          <Button variant="secondary" size="lg" onPress={() => onManageReps(competitionId)}>
            <Users size={20} color={colors.forest[700]} />
            代表者招待
          </Button>
          <Button variant="secondary" size="lg" onPress={() => onShowQR(competitionId)}>
            <QrCode size={20} color={colors.forest[700]} />
            QRコード発行
          </Button>
          <Button variant="outline" size="lg" onPress={() => onEdit(competitionId)}>
            <Edit3 size={20} color={colors.forest[700]} />
            コンペ編集
          </Button>
        </View>
      </ScrollView>
