...
async def check_account(device_id: str = Depends(get_device_id)):
    sb = get_supabase()
    res = fetch_single(sb.table("accounts").select("device_id, account_name, created_at").eq("device_id", device_id).maybe_single())
    if not res.data:
...
async def check_account_name(body: CheckNameRequest):
    ...
    sb = get_supabase()
    res = fetch_single(sb.table("accounts").select("account_name").eq("account_name", name).maybe_single())
    if res.data:
...
async def register_account(body: RegisterAccountRequest, device_id: str = Depends(get_device_id)):
    ...
    conflict = fetch_single(sb.table("accounts").select("account_name").eq("account_name", name).maybe_single())
    if conflict.data:
...
async def update_account(body: UpdateAccountRequest, device_id: str = Depends(get_device_id)):
    ...
    sb = get_supabase()
    conflict = fetch_single(sb.table("accounts").select("account_name").eq("account_name", name).neq("device_id", device_id).maybe_single())
    if conflict.data:
        return JSONResponse(status_code=409, content={"error": "このアカウント名は既に使用されています", "suggestions": generate_name_suggestions(name)})
...
async def get_dashboard(device_id: str = Depends(get_device_id)):
    ...
    account = fetch_single(sb.table("accounts").select("device_id, account_name, created_at").eq("device_id", device_id).maybe_single())
    if not account.data:
        raise HTTPException(status_code=404, detail="アカウントが見つかりません")
...
async def get_competition_detail(comp_id: str):
    sb = get_supabase()
    comp = fetch_single(sb.table("competitions").select("id, device_id, name, date, course_name, status, created_at").eq("id", comp_id).maybe_single())
    if not comp.data:
...
async def update_competition(comp_id: str, body: UpdateCompetitionRequest, device_id: str = Depends(get_device_id)):
    sb = get_supabase()
    comp = fetch_single(sb.table("competitions").select("device_id").eq("id", comp_id).maybe_single())
    if not comp.data:
        raise HTTPException(status_code=404, detail="コンペが見つかりません")
...
async def get_evidence_by_competition(comp_id: str, device_id: str = Depends(get_device_id)):
    ...
    is_rep = False
    if not is_owner:
        rep = fetch_single(sb.table("competition_representatives").select("id").eq("competition_id", comp_id).eq("representative_id", device_id).eq("status", "approved").maybe_single())
...
async def request_representative(comp_id: str, body: RequestRepresentativeRequest, device_id: str = Depends(get_device_id)):
    ...
    comp = fetch_single(sb.table("competitions").select("id, device_id").eq("id", comp_id).maybe_single())
    if not comp.data:
        raise HTTPException(status_code=404, detail="コンペが見つかりません")
    if comp.data["device_id"] == rep_device_id:
        raise HTTPException(status_code=400, detail="自分のコンペには代表者申請できません")

    target = fetch_single(sb.table("accounts").select("device_id").eq("device_id", rep_device_id).maybe_single())
    if not target.data:
        raise HTTPException(status_code=404, detail="指定されたアカウントが見つかりません")

    existing = fetch_single(sb.table("competition_representatives").select("id, status").eq("competition_id", comp_id).eq("representative_id", rep_device_id).maybe_single())
    if existing.data:
...
async def update_representative(rep_id: str, body: UpdateRepresentativeRequest, device_id: str = Depends(get_device_id)):
    ...
    rep = fetch_single(sb.table("competition_representatives").select("id, competition_id, representative_id, status, competitions:competition_id ( device_id )").eq("id", rep_id).maybe_single())
    if not rep.data:
    ...
    if body.status == "approved":
        ...
        existing_friend = fetch_single(sb.table("friendships").select("id").or_(f"and(eq.account_id,{owner_id},eq.friend_id,{rep_dev}),and(eq.account_id,{rep_dev},eq.friend_id,{owner_id})").maybe_single())
...
async def get_evidence_detail(evidence_id: str):
    sb = get_supabase()
    res = fetch_single(sb.table("evidence_images").select("id, competition_id, device_id, award_type, hole_number, distance, image_url, memo, created_at, competitions:competition_id ( name, date, course_name ), accounts:device_id ( account_name )").eq("id", evidence_id).maybe_single())
    if not res.data:
...
async def create_evidence(
    ...
    comp = fetch_single(sb.table("competitions").select("id").eq("id", competition_id).maybe_single())
    if not comp.data:
        raise HTTPException(status_code=404, detail="コンペが見つかりません")
...
async def delete_evidence(evidence_id: str, device_id: str = Depends(get_device_id)):
    sb = get_supabase()
    evidence = fetch_single(sb.table("evidence_images").select("id, device_id, image_url").eq("id", evidence_id).maybe_single())
